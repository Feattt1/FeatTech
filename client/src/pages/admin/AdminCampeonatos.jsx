...
const ESTADO_CONFIG = {
  ...
  FINALIZADO:    { label: 'Finalizado',     color: 'bg-slate-100 dark:bg-slate-800 text-slate-500 border-slate-200 dark:border-slate-700' },
};
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-slate-100">Torneos</h1>
          {!loading && (
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {campeonatos.map((c) => {
            ...
            return (
              <div key={c.id} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-slate-300 transition overflow-hidden">
                <div className="p-4 sm:p-5">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="font-semibold text-slate-900 dark:text-slate-100 text-base sm:text-lg truncate">{c.nombre}</h3>
                  ...
                  <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-slate-100 dark:border-slate-700">
                    ...
                    <Link
                      to={`/campeonatos/${c.id}`}
                      className="px-4 py-2 rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400 font-medium text-sm transition"
                    >
                      Ver público
                    </Link>
                    <button
                      ...
                      className="px-4 py-2 rounded-lg bg-white dark:bg-slate-800 border border-red-200 text-red-600 hover:bg-red-50 font-medium text-sm disabled:opacity-40 transition ml-auto"
                    >
